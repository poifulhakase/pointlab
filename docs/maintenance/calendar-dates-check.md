...
> 🆕 **2026-09-20：忘れても気づけるようにした**。`npm run check-calendar` が
> ①祝日の判定を**内閣府の公表CSVと突き合わせ**、②手で足した日程が**いつ尽きるか**を見る。
> 残り90日を切ったら失敗し、`fetch-data.yml`（土曜）経由で Discord の #エラー・異常 に出る。
> 🔴 日程は尽きても**エラーにならず静かに何も出なくなる**＝「イベントの無い週」と
> 見分けがつかないので、切れる前に知らせるのが要点。
