# swing-lab の日次実行（SPEC 13）。
#
# 起動 → パイプライン実行 → 通知 → 終了 の一発完結型。常駐しない。
#
# 🔴 ぽいロボのデータを先に最新化する。swing-lab は public/data/*.json を**読むだけ**で、
...
$root = Split-Path -Parent $PSScriptRoot          # swing-lab
...
try {
    ...
    #    Python 側のログは stdout に出している（swinglab/logs.py）。
