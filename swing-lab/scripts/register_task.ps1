# swing-lab をWindowsタスクスケジューラに登録する（SPEC 13）。
#
# 🔴 クラウド（GitHub Actions 等）では動かない。swing-lab は手元の `.env` と
...
param(
    [string]$Time = "17:00",
    [string]$TaskName = "swing-lab-daily",
    [switch]$Remove,
    [switch]$Show
)
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger `
    -Settings $settings -Description "swing-lab 日次実行（引け後）" -Force | Out-Null
