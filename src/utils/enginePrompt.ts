...
## データ品質チェック
4. **sanity_warnings に「価格基準日乖離」または「当日価格乖離」が含まれる場合**：MA乖離率・需給推進力は旧日付の終値ベース。先物最新終値との差分を踏まえ、実態との乖離を定性的に補正して解釈する。（例：「先物は+3%急騰しているが、MA乖離率はその前日値ベースであるため実際の乖離はさらに大きい可能性がある」）
5. **sanity_warnings に「USD/JPY 鮮度注意」が含まれる場合**：ドル円指標（需給推進力全体の30%ウェイト）が古い終値ベース。ドル円変動が大きい局面では需給推進力を補正して解釈する。`
...
JSON変数名・英語識別子は出力に一切含めない（「日本語ラベル一覧」に従うこと）。
