...
- **null の場合（sanity_ok=false）**：需給サマリー冒頭に「需給推進力・執行停止: [sanity_warnings を列挙]」を記載し、以降の分析はCOT・VIX・MA乖離・フロー・塩漬けリスク のみで構成する。確信度は最大50%とする。詳細は出力末尾の【データメモ】に記載する。【データ品質起因の上限。ステータス起因の固定値とは別ルール】
