...
export function buildStockAnalysisPrompt(
  ...
  return [
    ...
    '',
    '### 総括の決め方（この手順どおりに）',
    '1. **解消日数が5日分未満** → 「重しが消えつつある」',
    '2. **5日分以上** → 原則「まだ重い」。'
      + '🔴 例外で「消えつつある」にできるのは、**直近3か月で買残が20%以上減っている**ときだけ。'
      + '（ピークからの累計解消率が高くても、**直近が止まっていれば重い**。'
      + '　いま居座っている量が問題であって、過去に減ったことは関係ありません）',
    '3. 数字が取れない → 「どちらとも言えない」',
    '',
    '🔴 **基準を満たさないのに「消えつつある」と書かないでください。**'
      + '理由文に「◯日分で基準を上回るものの」と書きながら「消えつつある」と結論するのは矛盾です。'
      + '基準を外れたなら「まだ重い」と書いてください。',
    '',
    '## 記入のしかた',
    ...
    '- 出来高は「直近1か月」と「過去1年」の両方を出し、**倍率で厚みが分かる**ようにしてください。'
      + '🔴 過去1年の平均が直接取れなくても、**月足の出来高から概算**できます（合計÷営業日数）。'
      + '安易に「不明」にしないこと',
    '- 🔴 **信用残は最新の週を使ってください**。1週古い数字で答えないこと',
