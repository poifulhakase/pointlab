...
const [chartSettingsOpen, setChartSettingsOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      const v = s.view
      if (v === 'spec' || v === 'legal' || v === 'manual' || v === 'backtest' || v === 'evals' || v === 'playbook' || v === 'original' || v === 'timemachine' || v === 'chartpattern' || v === 'daytrade') { a.setView('support'); return }
    ...
    <div style={styles.app}>
      ...
      <div style={styles.body}>
        ...
        <main style={styles.main}>
          ...
          {cal.view === 'playbook' && (
            <ErrorBoundary label="戦略プレイブック">
              <Suspense fallback={<ViewLoader />}>
                <StrategyPlaybookPanel theme={theme} isMobile={isMobile} onClose={() => setViewWithTransition('support')} />
              </Suspense>
            </ErrorBoundary>
          )}
          {cal.view === 'original' && (
...
// 🔴 2026-08-09: 「エンジン」⇔「シールド」の名称を入れ替えたうえで、タブ構成を最終形にした。
//    シールド画面（'quant'）= シールド / 環境 / 現物 / 先物 / 周期（市場を見る道具を1画面に）
//    エンジン画面（'shield'）= ロボ口座（疑似トレード）1枚。**タブを持たない**
