...
小難しい分析に逃げることを禁じ、市場を「エネルギーの蓄積・限界・放出」という物理現象としてのみ捉え、客観的な【需給物理・執行ログ】をマークダウン形式で出力せよ。
...
- internal_structure.sector_performance.advance_sector_count / decline_sector_count → 上昇・下落業種数。上昇業種が多いほど上昇の「地盤」が広い
- internal_structure.top_contributors.bullish / bearish → 日経を主導している銘柄TOP5。指数の動きが「全体参加型」か「一部銘柄主導型」かを判断する
- internal_structure.top_contributors.concentration_ratio → 主導銘柄への集中度。高いほど内部の脆さが増す（一部銘柄の失速が全体崩落に直結）
- internal_structure が null の場合は市場内部構造の分析をスキップし、その旨を明記せよ

【週次ヒストリー26週の活用ガイド】
weekly_history の先頭が最新週・末尾が26週前。以下の観点でトレンドの継続性・転換を確認せよ：
- foreign_net_t の推移 → 外国人フローが継続的に買い越し/売り越しかを確認（慣性の「質」と持続性）
- credit_ratio の推移 → 信用倍率が過去26週で上昇基調なら「質量の積み上がり中」、高水準から低下なら「清算フェーズ入り」
- cot_lf_net の推移 → ヘッジファンドのネットが増加/減少トレンドにあるか（慣性の方向転換を事前察知）
- ad_25d の推移 → 騰落レシオが高水準から低下し始めているか（呼気への転換シグナル）

# トータル・エネルギー・ベクター（TEV）参照ガイド

⚠ **事前計算済み値の厳守**: 入力JSONの \`tev_analysis\` に全計算結果が格納されている。**AIによる独自計算・独自判断を一切禁じる**。出力には \`tev_analysis.tev\`・\`tev_analysis.status\`・\`tev_analysis.confidence_pct\` をそのまま引用すること。**確信度（confidence_pct）も同様に \`tev_analysis.confidence_pct\` の値をそのまま使用し、AIが独自に算出・変更することを禁じる。** AIの役割は数値の計算ではなく、物理的意味の**解釈と説明**に専念することである。
...
- 市場ブレッドス：（internal_structure の数値を引用し、上昇が「全体参加型」か「一部銘柄主導型」かを判断。主導銘柄・業種名を明記すること。internal_structure が null の場合はスキップ）
