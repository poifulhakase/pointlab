...
// ── NS倍率 水準ラベル・配色 ───────────────────────
// 8.5以上: 酷暑（赤）/ 8.0〜8.5: 真夏日（オレンジ）/ 7.5〜8.0: 適温（白黄）/ 7.5未満: 冷え込み（青）
...
// ── 日経・TOPIX フォーマット ──────────────────────
function fmtNikkei(v: number): string {
  return v.toLocaleString('ja-JP', { maximumFractionDigits: 0 })
}
function fmtSp500(v: number): string {
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 3 }}>
              <span style={{ fontSize: 10, color: 'var(--text-dim)' }}>
                日経 {fmtNikkei(latest.nikkei)}
              </span>
              <span style={{ fontSize: 10, color: 'var(--text-dim)' }}>
                S&amp;P {fmtSp500(latest.sp500)}
              </span>
              {updatedAt && (
