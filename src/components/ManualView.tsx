...
const SECTIONS: Section[] = [
    ...
    subs: [
      {
        subtitle: 'シールドタブ',
        items: [
          '「ぽいロボ シールド」エリアの「クオンツ分析用プロンプト＋需給データをコピー」ボタンを押すと需給データ入りのプロンプトがコピーされます。Gemini・Claude・ChatGPT・DeepSeek などの AI に貼り付けて分析を依頼してください。右側のテキストエリアは AI 分析レポートを保存するメモ欄です。',
  ...
  },
  {
    ...
    items: [
      ...
      '🔵 この画面は日経平均の話ではないため、シールド（需給分析）とは切り離した別ページにしています。',
    ],
  },
  {
    id: 'lab',
  ...
  },
  {
    ...
    items: [
      ...
      'ぽいロボは金融商品取引業（投資助言・代理業を含む）の登録を受けておらず、特定の銘柄等についての売買の助言やシグナルの配信は行いません。シールド・エンジン・戦略プレイブックなどが示す指標・需給状態・手法は、過去データにもとづく客観的な情報および教育であり、売買の推奨・指示ではありません。',
