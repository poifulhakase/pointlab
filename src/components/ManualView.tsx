...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    title: 'ぽいロボ（信用期日）',
    tocLabel: 'ぽいロボ',
    // 🔴 2026-09-16：疑似トレード（口座・成績・履歴）を廃止し、中身を「信用期日」に置き換えた
    intro: [
      '画面下のナビバーにある「ぽいロボ」アイコン（ロボット）をタップすると表示されます。',
      '🔴 コミュニティメンバー限定です。登録した銘柄はログインしたアカウントに保存されます。',
      '銘柄コードを入れて「作成」を押すと、その銘柄の週足と信用買残のチャートが右側に登録されます。登録した銘柄は「更新」で最新にし、「削除」で外せます。',
    ],
    subs: [
      {
        subtitle: 'チャートの見方',
        items: [
          '赤い帯＝需給が悪化した期間です。制度信用の買残が前週より増え、同じ週に株価が下がった週が3週以上続いたところに付きます。',
          '青い帯＝その6か月後（制度信用の期日の目安）です。休場日にあたるときは前の営業日にしています。下げの中で積み上がった信用買いは、この時期に決済されて売りが出尽くしやすくなります。',
          '下段は買残です。濃い色が制度信用、薄い色が一般信用です。一般信用は期日が長いか無期限のため、判定には使っていません。',
          '🔴 信用残は JPX の週次データを 2026年8月14日の週から貯めています。それより前の期間は判定できず、判定できる期間は毎週のびていきます。',
          '🔴 状態を記述したもので、売買の推奨ではありません。',
        ],
      },
    ],
  },
  ...
  },
  {
    id: 'believe',
    icon: '🤖',
    title: 'Future（第4次産業革命）',
    tocLabel: 'Future',
    intro: [
      '研究室のメニュー「Future / 第4次産業革命」から表示されます。',
      'ロボットが第4次産業革命を起こす、という見立てで選んだ銘柄を並べているページです。数ヶ月〜年単位で見る前提で、途中の上下は判定材料にしていません。',
      '🔴 このページは研究の記録であり、売買の推奨ではありません。ほかの画面の判定にも使っていません。',
    ],
    subs: [
      {
        subtitle: '選び方',
  ...
  },
  {
    ...
    items: [
      ...
      'ぽいロボは金融商品取引業（投資助言・代理業を含む）の登録を受けておらず、特定の銘柄等についての売買の助言やシグナルの配信は行いません。ブンセキ・信用期日・戦略プレイブックなどが示す指標・需給状態・手法は、過去データにもとづく客観的な情報および教育であり、売買の推奨・指示ではありません。',
