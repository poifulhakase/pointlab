...
const SECTIONS = [
  {
    ...
    items: [
      ...
      '日付をタップするとメモパネルが開きます。テキストメモ・チェックリストのほか、開始〜終了時刻とタイトル付きのスケジュールを複数追加できます。',
      'タイトル未入力のまま閉じると、空のスケジュールは自動的に破棄されます。',
  ...
  },
  {
    ...
    items: [
      ...
      '右上の「ぽいロボ エンジン（レンチアイコン）」→「プロンプト＋データをコピー」を押すと、AI分析用のプロンプトとデータがクリップボードにコピーされます。',
      'モーダル内の Gemini / Claude / ChatGPT ボタンをタップするとAIチャットが直接開きます（Android PWA対応）。コピーしたテキストをそのまま貼り付けるだけです。',
    ],
  },
  {
    icon: '🧪',
    title: '研究室',
    items: [
      'ナビバーのフラスコアイコンをタップすると研究室が表示されます。',
      '左パネルのメニューから「DATA（需給分析）」「REPORT（資料）」「SETTINGS（設定）」「HOME（カレンダー）」「CHART（チャート）」へ移動できます。',
      '「REPORT」で記事・解説資料を閲覧できます。右上の「×」ボタンで資料トップへ戻れます。',
      ...
      '右下の「ぽいロボ コネクト」ボタンをホバーすると説明パネルが表示されます。Googleログイン後にボタンを押すと、ぽいふる博士との音声通話・画面共有セッションが始まります。',
      '通話中はメニューが非表示になり、画面中央に通話パネルが表示されます。マイク・画面共有・切断ボタンのみ表示されます（iOS は画面共有非対応）。',
      '同時接続は最大2名までです。満員の場合は「現在満員です」と表示されます。',
    ],
  },
]
