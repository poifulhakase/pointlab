...
const SPEC_SECTIONS = [
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          'シールド（需給分析）：シールド / 周期 の2タブ ＋ AI分析プロンプト自動生成',
          'エンジン：エンジン / 環境 / 現物 / 先物 の4タブ。保有中ポジション管理・出口戦略（NK225先物データ＋AIプロンプト生成）＋市場データ',
  ...
  },
  {
    ...
    content: [
      {
        type: 'para' as const,
        text: '需給分析に特化したビューです。分析・環境・現物・先物の4タブ構成。分析タブは2カラム（ぽいロボ シールド / エントリー分析レポート）、環境・現物タブは3カラムレイアウト（各1/3幅）、先物タブは左2/3（週次分析）＋右1/3（日次テーブル）の横分割。市場の過熱感・資金動向を把握します。スマートフォンでは縦1列に並びます。',
      },
      {
        type: 'table' as const,
        headers: ['タブ', '左', '中', '右'],
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          'タブ: カレンダー / シールド / エンジン / チャート / 研究室（5タブ・アイコンのみ表示）',
  ...
  },
  {
    ...
    content: [
      {
        type: 'para' as const,
        text: 'フッターナビの「エンジン」タブからアクセスできるビューです。保有中ポジションの管理・出口戦略に特化し、エントリー判断は対象外です。NK225先物マーケットデータ（OHLCV・MA・建玉残高・PCR・VIX）を自動取得・表示し、AIプロンプトと保有ポジション画像を組み合わせて分析します。',
      },
      {
        type: 'list' as const,
      ...
      {
        type: 'list' as const,
        heading: 'エンジン AIプロンプト仕様',
        items: [
          'Role: 保有中ポジションの状態を整理する分析補助OS「ぽいロボ エンジン」（★2026-08-09 に「シールド」から改称）',
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '結果: **シールド（需給分析・`\'quant\'`）＝ シールド / 周期 の2タブ**、**エンジン（ポジション分析・`\'shield\'`）＝ エンジン / 環境 / 現物 / 先物 の4タブ**',
