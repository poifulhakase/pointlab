    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          'TradingView の開き方（★2026-08-08追加）: 一般ユーザー＝`https://jp.tradingview.com/symbols/TSE-{コード}/?timeframe=1M`（銘柄ページ・1か月足）。**管理者（isAdminEmail）のときだけ**＝`https://jp.tradingview.com/chart/{保存済みレイアウトID}/?symbol=TSE%3A{コード}`（自分のチャートレイアウトで開き、足やインジケーターはレイアウト側の設定に従う）。🔴 レイアウトIDは**個人のもの**なので一般ユーザーには使わない（他人が開くと管理者のインジケーター構成が見えるため）。判定は `utils/admin.ts` に集約。🔵 規約面＝外部リンクは TradingView 規約の制限対象外（制限されるのはコンテンツ/データの自前表示と自動アクセス）。他人が開いても有料プランは渡らず、機能制限は開いた人のアカウントで判定される',
