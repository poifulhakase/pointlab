...
const SPEC_SECTIONS = [
  ...
  },
  {
    ...
    content: [
      {
        type: 'para' as const,
        text: '需給分析に特化したビューです。環境・現物需給・先物需給の3タブ構成で、各タブは3カラムレイアウト（各1/3幅）を採用。市場の過熱感・資金動向を把握します。スマートフォンでは縦1列に並びます。',
      },
      {
        type: 'table' as const,
        ...
        rows: [
          ...
          ['現物需給', '信用倍率', '投資主体別売買動向', '需給指標（騰落レシオ・空売り・裁定）'],
          ['先物需給', '先物参加者別ネット枚数＋ベクター分析（2/3幅）', '←', '先物OI（建玉残高）'],
        ],
      },
      {
        type: 'list' as const,
      ...
      {
        type: 'list' as const,
        heading: '先物需給タブ ─ 先物参加者別ネット枚数・ベクター分析（MicroQuantView）',
        items: [
      ...
      {
        type: 'list' as const,
        heading: '先物需給タブ ─ 先物OI（FuturesOiPanel）',
        items: [
          'データソース: JPX 投資部門別売買高（OI合計）週次 → public/data/futures_oi.json',
          '表示内容: 全体建玉残高（ネット枚数合計）・前週比増減（Δ）',
          '直近60件表示（スマートフォンでは初期10件、「全件表示」ボタンで展開）',
          'キャッシュ: localStorage 24時間（`poical-futures-participants-v2` と同一取得フロー）',
        ],
      },
      {
        type: 'list' as const,
    ...
    content: [
      {
        ...
        rows: [
          ...
          ['poical-futures-participants-v2', '24時間', '先物投資部門別ネット枚数データ（先物需給タブ）'],
        ],
      },
      {
        type: 'list' as const,
