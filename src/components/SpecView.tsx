    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '🔵 **DATA→未来ガジェット→波動の書（旧「フォーメーション分析」★2026-08-11 新設／2026-08-13 改称）**＝**巻一・形**＝チャートパターン16種をSVGで図示（線が描かれる演出つき）。🔴 主張は「パターンを覚えよう」ではなく、**26年の日経で測ったら方向すら安定しなかった**という実測を併記すること。三尊は9通りの定義で先20日が +1.15% 〜 −1.34% とバラバラ。ダブルボトムだけが9通りすべてプラスだが基準(t≧2)は1通りのみ。未測定のものは「未測定」と書き、効くとも効かないとも書かない。ヘッダーはタイムマシンと同形、立場と測り方は ? に畳んである。🆕 **巻二・波＝エリオット波動**（推進5波＋修正3波の全体像／三つの掟／修正の型3種／目安）。🔴 巻を分ける理由＝形は機械的に定義できて測れたが、波は**数え方が一意に決まらず同じやり方で測れない**。だから**判断には使わない**（★2026-08-13 立場の断り書き2枚はユーザー判断で削除・巻を分けている事実で区別する）。🆕 **巻三・灯＝ローソク足の型18種**（1本6/2本6/3本以上6）。酒田五法の三川・三空・三兵・三法はここ、三山は三尊なので巻一。🔵 ローソクは成立日が機械的に決まるので巻一と同じ土俵で測れる（実測はまだ入れていない）',
