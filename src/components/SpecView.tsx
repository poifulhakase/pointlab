    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '移行期は両論併記（★2026-08-08）: 🔴 **移行期は現在地を1つに確定させない**。逆金融相場のあとの移行期では、ハードランディングなら逆業績相場を経由し、ソフトランディングなら素通りする。**どちらかは事後にしか確定しない**（NBERも内閣府の景気基準日付も事後認定／逆イールドは2022〜23年に外れた／正解ラベルが無く検証もできない）ので、判定を作らず両方を出す。表示＝中央に「移行期」＋「ハード→逆業績を経由／ソフト→素通り」、扇には逆業績相場に「◀ 経由？」・金融相場に「◀ 行き先」。🔴 **移行期は丸マーカーを出さない**（確定していないものに丸を打つと確定したように見える）。🔵 **弧は1本**＝直前のアンカーから行き先まで引く。ハードもソフトも**行き先は同じ**なので2本にしても情報が増えず、同じリング上で重なって読めない。外周にもう1本追加する案も、SIZE=344で余白が無くスマホ(320px)で円環が縮むため不採用。🔵 「次に来るとされる業種」は経路によらず金融相場なので変更なし',
