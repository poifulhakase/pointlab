    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '円環の中央＝表示切替ボタン（★2026-08-08）: 円の穴をクリックすると「**現在地**」と「**一致度**」を切り替える。両方を同時に出すと扇が3行になって重なるため、モードごとに出し分ける。🔵 **現在地モード**＝中央にマクロの現在地、扇に「◀ いまここ」「◀ 行き先？」、丸マーカー（実線＝確定／破線＝行き先の候補）、下の説明は「◯◯相場とは？」。🔵 **一致度モード**＝中央に一致度1位、扇に「一致度 ◯◯」、丸マーカーなし、下の説明は「景気の局面で物色対象が回るという見方 × 実測」＋一致度の「?」。🔴 上部に常設していた説明文と「?」は**この位置に移した**（モードで役割が変わるため）。既定は現在地モード。穴の下端に「クリックで◯◯へ」と小さく出して切替に気づけるようにしている。🔵 移行期の構造＝**現在地は直前のアンカー（確定・実線の丸）／行き先は2つの候補（未確定・破線の丸）**。逆金融相場のあとなら「逆業績相場 か 金融相場」（ソフトランディングなら逆業績を素通り）',
