...
const SPEC_SECTIONS = [
    ...
    content: [
      ...
      },
      {
        ...
        rows: [
          ...
          ['資料', 'NoteView — 記事一覧（基礎・インジケーター・イベントドリブン・未来ガジェット）。特殊カード: 需給エネルギーバックテスト（一般ユーザーも閲覧可）/ プロンプト Evals（管理者のみ）。「← 研究室」ボタンで研究室へ戻れる'],
      ...
      },
      {
        ...
        items: [
          ...
          'フローティングサブバー（右下 position:fixed zIndex: 150）: カレンダー(月/週/日)・チャート(銘柄)・エンジン(分析/環境/現物/先物)・シールド(シールド/ニュース)でタブ表示。各ビューのアクティブタブを右下 pill で切り替え（★2026-05-27 シールドタブ追加）',
        ],
      },
      {
        type: 'list' as const,
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'list' as const,
        heading: 'シールド画面構成 ★2026-05-27 ニュースタブ追加',
        items: [
          'タブ切り替え: 右下フローティングpill（App.tsx の floatSubBar に統合）でシールド / ニュース を切り替え。cal.view === "shield" のときのみ表示',
          ...
          '【ニュースタブ】NewsPanel: シールドパネルと同デザイン（サイバーターミナル風）。NEWS_STATUS_LINES（10行）をタイプライターアニメーションで表示。右側エリアは未実装（予約済み）',
          '【ニュースタブ】ヘッダー: 「ぽいロボ ニュース」＋メガホンSVGアイコン。COPYボタンはニュースプロンプトをコピー（copyStatus: "news_shield"）',
