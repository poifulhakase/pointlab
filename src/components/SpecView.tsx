    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '中身: 疑似元本100万円で、ブル・ベアETF 4本（1321 日経225連動 / 1570 日経レバレッジ / 1571 日経インバース / 1357 日経ダブルインバース）だけを売買する自律トレーダー。毎営業日の引け後（19:30 JST）に GitHub Actions が判断し、結果を `public/data/robo_account.json` に書く。**画面は読むだけで、書き込みは一切しない**',
          ...
          '🔴 **約定は実際の寄値で記録する（★2026-08-11）**＝08:30の判断は前営業日の終値を見て下すが、買えるのは09:00の寄値。実測で1570の窓（前日終値→寄値）は平均2.08%・上位10%で4.82%あり、終値で約定したことにすると成績が実物と別になる。判断した日は `account.pending` に積むだけにして、次の実行で日足に入った当日の始値で約定させる（1日遅れて確定）。損切りも判定だけ行い、決済は翌朝の寄値。🔵 **窓ガード**＝寄値が「判断時点の終値から引いた損切り値」の向こう側なら建てない（前提が寄り付きで壊れている）。通し確認では、同じ値動きで旧＝+20,000円の勝ち／新＝−26,000円の負けと**逆に出ていた**',
