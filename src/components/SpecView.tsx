    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          'セクター画面の操作（★2026-08-07）: 円環の扇をクリック＝その局面を選ぶ（局面が進むわけではない）。左下「◯◯相場とは？」の「?」＝局面の説明モーダル（**理論のみ**。順位・騰落率は入れない）。上部の「?」＝一致度の説明モーダル。検索結果は**行のどこを押しても銘柄コードをコピー**（貼り先は TradingView 想定）＋AI分析パネルを開く。「次に来るとされる業種」「いま資金が向かっている業種」の行クリックでその業種の銘柄を検索。「いま資金が向かっている業種」は既定でプラスの上位5件のみ（全17業種はボタンで展開）',
