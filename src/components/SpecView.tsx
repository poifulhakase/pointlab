    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '局面の「型」との一致度（★2026-08-07追加）: セクター画面で、景気4局面それぞれについて「その局面で強いとされる業種が実際に17業種中どのあたりに並んでいるか」だけを見て 0〜100 を算出（上位独占=100／下位独占=0）。局面ごとに業種数が3〜6と異なるため、取りうる最良・最悪の平均順位で正規化。🔵 平均騰落率ではなく順位で見るのは、突出した1業種にグループ全体の評価を持っていかれないため。上位1/3=支持／下位1/3=矛盾として根拠も表示。1か月/3か月/6か月を同時表示（切替だと「最近型が変わった」が見えない）。🔴 **これは確率ではない**（局面に正解ラベルが無く、当たったかを検証できないため）。画面とAIプロンプトの両方に明記し、AIには確率への言い換えを禁止。🔴 ぽいロボは依然として局面の判定・断定は行わない',
          '業種別の相対強弱（★2026-08-07追加）: NEXT FUNDS TOPIX-17 業種別ETF（1617〜1633）の Yahoo Finance 日足 → public/data/sector_perf.json。17業種の1か月/3か月/6か月騰落率と順位。🔴 業種別株価指数そのものは有料配信のため**ETFによる代用**（proxy:"etf" を出力に明記）。🔴 騰落率は**調整後終値(adjclose)**で計算＝分配金の権利落ち（1回0.7〜2%）が業種ごとにバラバラに乗って順位が入れ替わるのを防ぐ。ETFコードと17業種コードは 1617+(n-1) で一対一（data_j.xls のETF名で検証・テストで固定）',
