...
const ACTIONS = [
  ...
  { n: '3', t: '年度末の最終2日は逆に下げる', s: '3月末は26年中18年がマイナス（配当落ち＋期末の益出し）。権利確定前までは強いが、最後の2営業日は別（2026-08-11 実測）' },
]
...
/** 循環（セクターローテーション）。サイドバーのバナーと同じ「回る」印象に寄せる。 */
function CycleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function StrategyPlaybookPanel({ theme, isMobile, onClose }: Props) {
  ...
  const slides: { id: string; node: React.ReactNode }[] = [
    ...
    { id: 'overview', node: slide('ぽいロボでの運用', '全体像・OVERVIEW', (
      <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 14 : 18 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 8 : 10, fontSize: isMobile ? 12.5 : 14, color: c.SUB, lineHeight: 1.9 }}>
          <p style={{ margin: 0 }}>ぽいロボは「相場のいま」を読む<b style={{ color: c.TEXT }}>道具</b>と、迷ったとき頼れる<b style={{ color: c.TEXT }}>伴走</b>でできています。</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 10 : 14 }}>
          {[
            ...
            { icon: <EngineIcon />,       nm: 'ぽいロボ',            ds: '信用買いの積み上がりと期日の目安を見る' },
    ...
    { id: 'week', node: slide('1週間の流れ', '土曜起点・ROUTINE', (
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {([
          ...
          ['3', '持っている間（管理）', '決めた損切り値を動かさないことが仕事。'],
    ...
    { id: 'shield-tool', node: slide('ぽいロボ', '信用買いの期日を見る・MARGIN CYCLE', toolBody(
      ...
      '見るのは「売りが出尽くす時期」',
      <>積み上がった信用買いは期日に決済されやすく、そこを過ぎると需給が軽くなる、という見立てです。🔴 <b style={{ color: c.TEXT }}>信用残は2026年8月から貯めているので、それより前は判定できません。</b>🔴 <b style={{ color: c.TEXT }}>コミュニティメンバー限定です。</b>状態の記述で、売買の推奨ではありません。</>,
    ), <EngineIcon />) },
