...
const DISCLAIMER_SECTIONS: LegalSection[] = [
  {
    ...
    content: [
      ...
      { type: 'para', text: 'ブンセキ・信用期日・戦略プレイブック等が示す指標・傾向・手法・スコア・判定等は、過去データにもとづく一般的・客観的な情報および教育であり、特定の売買のシグナル配信や推奨ではありません。投資の最終判断は、必要に応じて外部 AI 等の分析を参考にしつつ、利用者ご自身の責任で行ってください。' },
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      { type: 'para', text: '本サービスのブンセキ機能は、需給データ・チャートデータを整形し、外部 AI（Claude・Gemini・ChatGPT 等）での分析を補助するためのツールです。' },
  ...
  },
  {
    ...
    content: [
      { type: 'list', heading: '本サービスの性質', items: [
        ...
        'ブンセキ・信用期日・戦略プレイブックその他が示す指標・傾向・手法・スコア・判定等は、過去データにもとづく一般的・客観的な情報および教育であり、特定の売買の推奨ではありません。',
