...
- お知らせフィード … [`NOTE_FEED.md`](NOTE_FEED.md)（note・ポイ活。トレードとは別系統）
...
```powershell
# ① ぽいロボのデータを最新にする（ロボトレード はこれを読むだけ）
...
ロボトレード が同じものを取りに行くと**同じ数字の出どころが2つ**になり、食い違ったとき
どちらが正か分からなくなる。だから `robotrade/data/poirobo.py` は読むだけ。
...
価格・損益・資産額はすべて **銭（1円の1/100）単位の int**（`robotrade/money.py`）。
...
└── robotrade/
    ...
    └── notify/
        ...
        └── feeds.py        note・ポイ活のお知らせ（おまけ・トレードとは別系統）
```
...
✅ **決算発表予定日は導入済み**（2026-09-20）。`robotrade/data/earnings.py`。
JPX公開サイトの Excel（**登録もAPIキーも不要**）。`config.yaml` の `earnings.enabled`。

- 🔴 **J-Quants の予定日APIは使わない**。無料枠でも取れるが
  **翌営業日ぶんだけ・3月期と9月期の会社だけ**（REIT除く）で、2〜14日スイングには足りない。
  JPX の一覧は決算期末の月ごとに**数週間先まで**載る（実測 680銘柄・約1か月先まで）。
- 使いどころは2つ:
  1. **第3層で除外** … 翌営業日から `screen.exclude_earnings_within_days`（既定3日）以内に
     決算がある銘柄は新規建てを落とす。🔴 保有期間ぶん（14日）全部を落とすと
     決算シーズンに候補が全滅するので短くし、少し先の決算は候補に残して判断AIに委ねる。
  2. **需給AIに実データを渡す** … `event_in_horizon` を推測させず、予定日と残り日数を渡す。
- 🔴 **「予定が無い」と「取れていない」を混ぜない**（前者は事実、後者は自分の未整備）。
- 🔴 予定は変更される。「未定」の行は捨てる（日付を捏造しない）。予定表の基準日も一緒に渡す。
- 🔴 **過去日の再実行では厳密な point-in-time にならない**（手に入るのは「いま時点」の予定表）。
  `as_of` より前の予定を返さないことで最低限の先読みは防ぐ。Phase 11 ではこの限界を承知で使う。
- 取れなくてもその日の実行は止めない（決算の判定だけ「データなし」に落ちる）。
- 予定表は `data/earnings_schedule.json` に貯める（3日で取り直し・gitには入れない）。

✅ **ニュース/適時開示（TDnet）は導入済み**（2026-09-20）。`robotrade/data/tdnet.py`。
`config.yaml` の `news.source: tdnet`（`none` で止められる）。無料・APIキー不要。

- 一次情報（発行体自身の開示）だけを使う。株探・Yahoo!ファイナンス等の
  **スクレイピングは規約で禁止**なので足さない。SNS・掲示板も使わない（SPEC 7.4）。
- 🔴 **判断日より後に公表された開示は渡さない**（`as_of`）。`--force` で過去日を
  再実行したときに未来のニュースを見て買う、という嘘の成績を防ぐ。
- 🔴 取得に失敗してもその日の実行は止めない（材料なしとして続ける）。
- 開示が1件も無い日は LLM を呼ばず `catalyst: none`。
  そのとき **「取得元が未導入」と「直近30日の適時開示なし」を書き分ける**
  （前者は自分の未整備、後者はその銘柄の事実）。
- タイトルだけ読む（本文PDFは読まない）。材料の種類はタイトルにほぼ出る。
