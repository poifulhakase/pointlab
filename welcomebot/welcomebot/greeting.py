...
#    2026-09-19 に運用者から文面が確定で届いたので、それだけにした
#    （それまでは3種類をランダムに出していた）。
# 🔵 増やしたくなったらこのリストに足すだけでランダムに戻る。
# 🔵 {name} は2か所に出る。差し込む前に safe_name() を通す。
GREETINGS: list[str] = [
    """やあ、{name} 君。ぽいふる博士だ。
ぽいんとらぼへようこそ！

諸君も今日から研究員のひとりじゃ。
まずは「やぁ諸君！」で自己紹介から始めてくれたまえ。
・ニックネーム
・興味のある分野（ポイ活／副業／投資／その他）
・今ハマっていること、挑戦したいこと

助手のぽよん君、発明品のぽいロボと共に、{name} 君を歓迎するぞ。
さらばじゃ！""",
]
...
def build_greeting(name: Any, *, mention: str = "", rng: random.Random | None = None) -> str:
    ...
    🔵 GREETINGS が複数あればランダムに選ぶ。いまは運用者が決めた1種類だけ。
    """
