...
| **`--dry-run`** | 初回シードも**書かない** | 「試しに動かしただけ」が本番の初期化になっていた（2026-09-19に踏んだ） |
...
🔴 4つめ: **`-AtLogOn` に利用者を指定しないと登録に管理者権限が要る**（Access is denied）。
`-User "$env:USERDOMAIN\$env:USERNAME"` を付けて自分のログオン時だけにする。
...
✅ **稼働中**（2026-09-19）。Bot `ぽいふる博士#7987` がサーバーに入り、
ログオン時に自動起動する常駐タスク `welcomebot` を登録済み。
既存メンバーはシード済みなので、**次に入ってきた人から**歓迎が飛ぶ。
テスト 24件 全green。

確認したこと:
- トークン・SERVER MEMBERS INTENT・招待（権限なし）
- 文面が `#やぁ諸君！` にどう出るか（1通出して削除）
- タスクが Running で、落ちても1分後に再起動する設定
